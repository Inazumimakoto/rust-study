...
## 7. 配列/vectorの範囲外アクセス（実験済み！）

### C++（地獄）

**警告は出るけどコンパイルは通る！実行もされる！**

```cpp
int arr[5] = {1, 2, 3, 4, 5};
std::cout << arr[100] << std::endl;  // 警告出るけど通る！

std::vector<int> vec = {1, 2, 3, 4, 5};
std::cout << vec[100] << std::endl;  // 警告すら出ない！
```

![vectorの範囲外アクセス - 普通に動いちゃう](../images/cpp_vector_ub.png)

**結果: `vec[100] = 0`** ← ゴミ値！でも動いてる！

| アクセス方法 | 何が起きる |
|-------------|-----------|
| `arr[100]` | 警告出るけど動く、ゴミ値 |
| `vec[100]` | 警告なし！ゴミ値 or クラッシュ |
| `vec.at(100)` | 例外（でも誰も使わない） |

### Rust（天国）

```rust
let arr = [1, 2, 3, 4, 5];
println!("{}", arr[100]);  // パニック！安全に停止！
