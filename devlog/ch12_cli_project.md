...
## 11. 標準出力と標準エラー出力 (`eprintln!`)

「正常な出力」と「エラーメッセージ」は混ぜてはいけない！

### なぜ分けるの？
*   `cargo run > output.txt` のようにリダイレクトした時：
    *   **標準出力 (`println!`)**: ファイル (`output.txt`) に書き込まれる。
    *   **標準エラー出力 (`eprintln!`)**: 画面（コンソール）に表示されたままになる。

こうすることで、「検索結果はファイルに保存しつつ、エラーが出たらすぐに画面で気づける」という便利な使い方ができる。
...
これで `minigrep` は一人前の CLI ツールになった！🎉
