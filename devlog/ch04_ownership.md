...
```rust
fn find_space(s: &String) -> usize {
    // スペースの位置を返す（例: 5）
}

let mut s = String::from("hello world");
let index = find_space(&s);  // 5

s.clear();  // 文字列を空に！

// index = 5 はもう意味がない！
// でもコンパイラは止めてくれない（ただの数字だから）
```

#### 解決: スライスを返すと安全！

```rust
fn first_word(s: &String) -> &str {  // スライスを返す
...
let mut s = String::from("hello world");
let word = first_word(&s);  // "hello" を借用

s.clear();  // ❌ コンパイルエラー！
            // word が借用中だから！
```

**スライスは借用だから、元を変えようとするとコンパイラが止める！**

### 参照 = 借用

| 用語 | 意味 |
|------|------|
| **参照（Reference）** | `&T`, `&mut T` という型 |
| **借用（Borrowing）** | 参照を通じて値を使うこと |
...
```rust
let s = "Hello, world!";  // s は &str 型
```

**なぜ？** → 文字列リテラルはバイナリに埋め込まれてる！
...
└──────────────────────┘
        ↑
let s = "Hello, world!";  // これを指す参照
```

| | `String` | `&str` |
|---|----------|--------|
| 場所 | ヒープ | バイナリ or Stringの一部 |
| 可変 | 可変可能 | 不変 |
| 所有 | 所有する | 借用 |
...
## おまけ: Move vs 借用 vs シャドーイング

似てるけど全然違う！整理しよう！

| | Move | 可変借用 (`&mut`) | シャドーイング |
|---|------|------------------|---------------|
| やること | 所有権を移す | 一時的に貸す | 新しい変数で隠す |
| 元の変数 | もう使えない | 借用終わったら使える | 隠れる（存在はする）|
| 型変更 | - | ❌ | ✅ できる！ |
| `let` | 不要 | 不要 | **必要！** |
...
- **スライス**: データの一部を参照（`&str`, `&[T]`）
- 文字列リテラルは `&str`（バイナリを指す参照）
- 「参照を取る」=「借用する」
