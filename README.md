# 🦀 Rust 勉強ノート

[The Rust Programming Language](https://doc.rust-lang.org/book/) を読みながら学習中！

> 🔥 **Rustを理解するには、C++がどれだけゴミかを理解すればいいだけ。**
> 
> → **[🖕 C++, FUCK YOU!](cpp_fuckyou.md)** ← 必読！

## 📊 進捗

| 章 | タイトル | 状態 |
|----|----------|------|
| 0 | [前書き](devlog/ch00_foreword.md) | ✅ |
| 1 | [入門](devlog/ch01_getting_started.md) | ✅ |
| 2 | [数当てゲーム](devlog/ch02_guessing_game.md) | ✅ |
| 3 | [一般的なプログラミング概念](devlog/ch03_common_concepts.md) | ✅ |
| 4 | [所有権](devlog/ch04_ownership.md) | ✅ |
| 5 | [構造体](devlog/ch05_structs.md) | ✅ |
| 6 | [Enum とパターンマッチ](devlog/ch06_enums.md) | ✅ |
| 7 | [パッケージ・クレート・モジュール](devlog/ch07_packages.md) | ✅ |
| 8 | [一般的なコレクション](devlog/ch08_collections.md) | ✅ |
| 9 | [エラー処理](devlog/ch09_error_handling.md) | ✅ |
| 10 | [ジェネリクス・トレイト・ライフタイム](devlog/ch10_generics.md) | ✅ |
| 11 | [テスト](devlog/ch11_testing.md) | ✅ |
| 12 | [I/Oプロジェクト: CLIツール](devlog/ch12_cli_project.md) | ⬜ |
| 13 | [イテレータとクロージャ](devlog/ch13_iterators.md) | ⬜ |
| 14 | [Cargo と Crates.io](devlog/ch14_cargo.md) | ⬜ |
| 15 | [スマートポインタ](devlog/ch15_smart_pointers.md) | ⬜ |
| 16 | [並行処理](devlog/ch16_concurrency.md) | ⬜ |
| 17 | [オブジェクト指向](devlog/ch17_oop.md) | ⬜ |
| 18 | [パターン](devlog/ch18_patterns.md) | ⬜ |
| 19 | [高度な機能](devlog/ch19_advanced.md) | ⬜ |
| 20 | [最終プロジェクト: Webサーバ](devlog/ch20_web_server.md) | ⬜ |

> ✅ = 完了 / 🔄 = 進行中 / ⬜ = 未着手

## 📁 フォルダ構成

```
rust-study/
├── README.md          ← このファイル
├── cpp_fuckyou.md     ← 🖕 C++ネガキャン（必読）
├── diary.md           ← 📔 学習日記
├── devlog/            ← 章ごとの学習メモ
├── images/            ← スクリーンショット
└── projects/          ← プロジェクト
    ├── ch01_hello_world/
    ├── ch01_hello_cargo/
    └── guessing_game/
```

## 🗓 学習開始日

2026-01-01

---

## 🎯 今後の目標

The Book 完走したら...

1. **🖥️ Rust で OS 開発** - [Writing an OS in Rust](https://os.phil-opp.com/)
2. **🌐 Rust で ICN スタック開発** - Information Centric Networking

**C++ の闇から解放されて、安全な低レベルプログラミングへ！** 🦀

